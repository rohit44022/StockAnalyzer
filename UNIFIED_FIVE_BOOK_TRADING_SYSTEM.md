# UNIFIED TRADING SYSTEM — Five Lenses, One Decision
# ══════════════════════════════════════════════════════
#
# This is ONE integrated trading system built from five complementary
# books. Each book adds a unique lens — together they form a single
# decision framework, not five separate systems.
#
# The Five Lenses:
#   [BOLLINGER]   — Volatility & mean-reversion (timing)
#   [MURPHY]      — Trend structure & confirmation (direction)
#   [BROOKS]      — Bar-by-bar price action (precision)
#   [WEIS]        — Volume-price behavior / Wyckoff method (intent)
#   [DALTON]      — Market Profile / Auction Theory (institutional value)
#   [CONFLUENCE]  — Where 2+ lenses converge (conviction)
#
# HONESTY NOTES:
# ──────────────
# [WEIS] teaches QUALITATIVE bar-by-bar reading. All numeric thresholds
#   are [CALIBRATION] — our quantification of his teaching.
# [DALTON] teaches CONCEPTUAL auction theory. Day types, Value Areas, and
#   POC require Market Profile / volume-at-price data to implement exactly.
#   From daily OHLCV we can approximate but not perfectly replicate.
# Phase names (ACCUMULATION, MARKUP, etc.) are [POST-WYCKOFF] course
#   terminology. Both Weis and Dalton note these were "added after Wyckoff."
#
# Every rule below is:
#   1. Specific and binary (YES/NO)
#   2. Labeled with its source lens
#   3. Part of a checklist executable in the specified time
#   4. Accompanied by a plain-English hint
#
# ══════════════════════════════════════════════════════


## 0. THE FIVE QUESTIONS — Before EVERY Trade
## ═════════════════════════════════════════════════════
##
## Ask these five questions BEFORE every trade. Each maps to one book/lens.
## Alignment count determines position sizing — or whether to trade at all.
##
## ┌──────┬───────────────────────────────────────────┬──────────┐
## │  #   │  Question                                 │  Lens    │
## ├──────┼───────────────────────────────────────────┼──────────┤
## │  Q1  │ Where is institutional value?             │ [DALTON] │
## │      │ (Am I buying in value or chasing?)        │          │
## │  Q2  │ What Wyckoff phase / volume intent?       │ [WEIS]   │
## │      │ (Is smart money accumulating or exiting?) │          │
## │  Q3  │ What is the macro trend structure?        │ [MURPHY] │
## │      │ (Are MAs, RSI, and structure bullish?)    │          │
## │  Q4  │ What is the volatility environment?       │[BOLLINGER│
## │      │ (Squeeze? Expansion? Band position?)      │          │
## │  Q5  │ What does bar-by-bar price action say?    │ [BROOKS] │
## │      │ (Always-in direction? Pattern quality?)   │          │
## └──────┴───────────────────────────────────────────┴──────────┘
##
## ALIGNMENT SCORING:
##   5 of 5 align → HIGHEST PROBABILITY TRADE — full size
##   4 of 5 align → HIGH PROBABILITY TRADE — 75% size
##   3 of 5 align → MARGINAL — 50% size, tight stops, quick exit
##   2 or fewer    → DO NOT TRADE
##
## The questions take < 60 seconds once you've done your pre-session work.


## 1. DAILY PRE-SESSION CHECKLIST (< 5 minutes)
## ═════════════════════════════════════════════════════
##
## Complete this BEFORE the session opens. This is your game plan.
## It feeds the Five Questions above with prepared answers.

### Step 1: Multi-Timeframe Context [DALTON + MURPHY] (60 seconds)

**Rule PS-1** [DALTON — Market Profile Layer] Review Longest to Shortest
  - Monthly bar: What is the long-term trend? Where are excess highs/lows?
  - Weekly bar: Is the market one-timeframing? (each week's low > prior week's low = one-TF up)
  - Daily bar: Recent direction? Any gaps? Inside days? Excess?
  - [MURPHY] Confirm with 50-SMA and 200-SMA: Above both = bullish. Below both = bearish.
  - Binary: Long-term UP / DOWN / BRACKETED
  - 📝 Hint: "Start zoomed out. Monthly → Weekly → Daily. Don't get
    lost in today's noise without seeing the big picture."

**Rule PS-2** [DALTON — Market Profile Layer] Identify Key Reference Points
  - Previous day's Value Area High (VAH) and Value Area Low (VAL)
  - Previous day's POC (fairest price)
  - Previous day's range high and low
  - Any unfilled gaps (gap = invisible single prints = strongest excess)
  - Bracket highs and lows (multi-day/week trading range extremes)
  - [MURPHY] Major support/resistance levels, round numbers, MA levels
  - 📝 Hint: "Mark the lines that matter. These are where trades live."

### Step 2: Prepare Three Scenarios [DALTON] (60 seconds)

**Rule PS-3** [DALTON — Market Profile Layer] Write Down Three Scenarios
  - Scenario A: Market auctions UP → What is the destination? What reference is overhead?
  - Scenario B: Market auctions DOWN → What is the destination? What reference is below?
  - Scenario C: Market stays in BALANCE → Trade responsively at extremes or stand aside
  - For each: What is your action? What confirms the scenario?
  - **This is the single most important preparation step.** Dalton: "Traders who
    don't prepare are at a serious disadvantage."
  - 📝 Hint: "If up → I do X. If down → I do Y. If flat → I do Z.
    Write it down. Don't improvise."

### Step 3: Overnight Inventory [DALTON] (30 seconds)

**Rule PS-4** [DALTON — Market Profile Layer] Assess Overnight Inventory
  - Is overnight trade mostly above or below yesterday's close?
  - Above = overnight inventory LONG → expect potential selling at open (adjustment)
  - Below = overnight inventory SHORT → expect potential buying at open (adjustment)
  - Neutral = no strong bias
  - [DALTON] If inventory is long/short and market does NOT adjust → STRONG signal
    in the non-adjustment direction (longs held = bullish, shorts held = bearish)
  - 📝 Hint: "If everyone went long overnight and the market doesn't sell off
    at the open, the bulls are VERY strong."

### Step 4: Wyckoff Phase + Volume [WEIS] (60 seconds)

**Rule PS-5** [WEIS] Name the Wyckoff Phase
  - ACCUMULATION / MARKUP / DISTRIBUTION / MARKDOWN
  - [POST-WYCKOFF] Phase names are course terminology.
  - Check: up-wave vs down-wave volume. Which side dominates?
  - Check: Is there shortening of thrust? Change in behavior?
  - Binary: Phase = ____. Volume character = DEMAND / SUPPLY / BALANCED.
  - 📝 Hint: "Is smart money buying or selling? What does the volume say?"

### Step 5: Volatility State [BOLLINGER] (30 seconds)

**Rule PS-6** [BOLLINGER] Check Squeeze / Band Width
  - Is BBW < 25th percentile of last 120 bars? = SQUEEZE (big move imminent)
  - Is price near upper/lower band? = possible mean-reversion or band-walk
  - Binary: SQUEEZE / NORMAL / EXPANDED
  - 📝 Hint: "Tight bands = storm coming. Which way? The other 4 lenses tell you."

### Step 6: Price Action Bias [BROOKS] (30 seconds)

**Rule PS-7** [BROOKS] Determine Always-In Direction
  - Would a trader who MUST always hold a position be LONG or SHORT right now?
  - Check: Last significant signal bar, trend channel, recent pattern
  - Binary: LONG / SHORT / FLAT
  - 📝 Hint: "If forced to bet, which way? That's your bar-by-bar bias."


## 2. PRE-TRADE CHECKLIST (< 60 seconds)
## ═════════════════════════════════════════════════════
##
## You've done your pre-session work. A potential trade appears.
## Run through these checks. If more than 2 fail → NO TRADE.

### A. Where Is Institutional Value? [DALTON]

**Rule 2.1** [DALTON — Market Profile Layer] Trade Location Assessment
  - Is the entry price ABOVE or BELOW today's developing POC/fairest price?
  - For LONGS: buying below POC = good location. Buying above = chasing.
  - For SHORTS: selling above POC = good location. Selling below = chasing.
  - [DALTON] "Where would you build a gas station — on a four-way corner off
    the interstate, or between traffic lights on a divided road?"
  - [CONFLUENCE with WEIS] If Wyckoff phase says BUY and location is below POC
    → Maximum confluence. If phase says BUY but location is above POC → Reduced size.
  - Binary: GOOD location / POOR location
  - 📝 Hint: "Am I getting a price that institutions would accept?
    Or am I chasing the crowd?"

**Rule 2.2** [DALTON — Market Profile Layer] Initiative vs Responsive Assessment
  - If buying within or above previous day's VA = **initiative buying** (strongest)
  - If buying below previous day's VA = **responsive buying** (reactive)
  - If selling within or below previous day's VA = **initiative selling** (strongest)
  - If selling above previous day's VA = **responsive selling** (reactive)
  - [CONFLUENCE with WEIS] Initiative activity + matching Wyckoff phase = MAXIMUM conviction
  - Binary: INITIATIVE (strong) / RESPONSIVE (opportunistic)
  - 📝 Hint: "Am I hunting (initiative) or reacting (responsive)?
    Hunters have more conviction."

**Rule 2.3** [DALTON — Market Profile Layer] Day Type Recognition
  - Identify the developing day type from the open:
  - Open-Drive → enter early, trust the direction (Rule D18)
  - Open-Test-Drive → enter after reversal confirmation (Rule D19)
  - Open-Rejection-Reverse → be PATIENT, wait for rotation (Rule D20)
  - Open-Auction → trade extremes responsively or stand aside (Rule D21)
  - Binary: Is this day type favorable for my intended trade? YES / NO
  - 📝 Hint: "What kind of day is developing? Don't force a trend trade on a Nontrend day."

### B. What Is the Phase + Volume Story? [WEIS]

**Rule 2.4** [WEIS] Phase + Direction Alignment
  - If ACCUMULATION or MARKUP → Only BUY setups
  - If DISTRIBUTION or MARKDOWN → Only SELL/EXIT setups
  - Does the intended trade direction MATCH the phase? YES = GREEN. NO = RED.
  - [POST-WYCKOFF] Phase names are course terminology.
  - 📝 Hint: "Don't buy what smart money is selling."

**Rule 2.5** [CONFLUENCE — WEIS + MURPHY] Volume Confirms Price?
  - [WEIS] Up-wave vol vs down-wave vol: DEMAND_DOMINANT (>1.5×) / SUPPLY_DOMINANT (<0.67×)
  - [MURPHY] On the last move: did volume expand with price?
  - Binary: Volume supports the trade = GREEN / Volume diverges = WARNING
  - 📝 Hint: "Is volume backing up my thesis?"

**Rule 2.6** [WEIS] Effort vs Result on Last Bar
  - HIGH volume + NARROW bar = ABSORPTION (someone blocking the move)
  - LOW volume + NARROW up bar = NO DEMAND
  - LOW volume + NARROW down bar = NO SUPPLY (bullish)
  - HIGH volume + WIDE bar + close in direction = GENUINE
  - Binary: Effort/result supports trade? YES / NO
  - 📝 Hint: "Lots of energy but no price movement? Someone big is absorbing."

**Rule 2.7** [WEIS] Check for Exhaustion / Change in Behavior
  - Last 3 same-direction waves covering LESS distance? = EXHAUSTION
  - Largest opposite-direction bar in 20 periods? = CHANGE IN BEHAVIOR
  - Either present = CAUTION. Both present = DO NOT ENTER in current direction.
  - 📝 Hint: "The move is running out of steam or the other side just woke up."

### C. What Does Structure Say? [MURPHY]

**Rule 2.8** [MURPHY] Moving Average Alignment
  - For BUY: Price > 20 SMA > 50 SMA. RSI > 50. MACD above signal.
  - For SELL: Price < 20 SMA < 50 SMA. RSI < 50. MACD below signal.
  - Binary: MAs aligned with trade = GREEN / Misaligned = WARNING
  - 📝 Hint: "Are the averages pointing the same way as my trade?"

### D. What Does Volatility Say? [BOLLINGER]

**Rule 2.9** [BOLLINGER] Band Position + Squeeze
  - In SQUEEZE → any breakout is significant. Direction from other 4 lenses.
  - Price at upper band in uptrend = possible band-walk (bullish continuation)
  - Price at lower band in downtrend = possible band-walk (bearish continuation)
  - Price touching opposite band = possible mean-reversion
  - Binary: Volatility state supports trade? YES / NO
  - 📝 Hint: "Where is price relative to the bands, and what does that mean?"

### E. What Does Price Action Say? [BROOKS]

**Rule 2.10** [BROOKS] Always-In + Pattern Quality
  - Is always-in direction matching intended trade?
  - Is there a signal bar / pattern (failed BO, wedge flag, double bottom/top)?
  - Binary: PA supports trade? YES / NO
  - 📝 Hint: "Does bar-by-bar action agree with my thesis?"

### F. THE FIVE-QUESTION ALIGNMENT COUNT [CONFLUENCE]

**Rule 2.11** [CONFLUENCE] Final Alignment Scoring
  Count how many of these five agree with your trade:
  1. Q1 [DALTON]: Good trade location relative to value? ☐
  2. Q2 [WEIS]: Phase + volume support direction? ☐
  3. Q3 [MURPHY]: Trend structure confirms? ☐
  4. Q4 [BOLLINGER]: Volatility state favorable? ☐
  5. Q5 [BROOKS]: Bar-by-bar price action agrees? ☐

  **5/5 = HIGHEST PROBABILITY → full position size**
  **4/5 = HIGH PROBABILITY → 75% size**
  **3/5 = MARGINAL → 50% size, tight stops, quick exit plan**
  **≤2/5 = DO NOT TRADE**
  - 📝 Hint: "Five green lights = go. Four = drive carefully.
    Three = pump the brakes. Two or less = park the car."


## 3. ENTRY RULES
## ═════════════════════════════════════════════════════

### A. BUY Entries

**Rule 3.1** [CONFLUENCE — WEIS + BROOKS + DALTON] Spring BUY (Maximum Confluence)
  - [WEIS] Price breaks below support → reverses → LOW volume on penetration
  - [BROOKS] Same event = failed breakout below range → reversal bar
  - [DALTON] Spring occurs at or near Value Area Low → best institutional location
  - [DALTON] If spring occurs at a bracket low (multi-day support) → HIGHEST conviction
  - [WEIS] Follow-through: Next 1-3 bars must continue UP with expanding volume.
    NO follow-through = signal disqualified.
  - [WEIS] Trend context: Spring in UPTREND = highest success. Spring in DOWNTREND = risky.
  - 📝 Hint: "A trap below support confirmed by follow-through at a value zone = gold."

**Rule 3.2** [CONFLUENCE — BOLLINGER + WEIS + DALTON] Squeeze Breakout BUY
  - [BOLLINGER] Squeeze ON (BBW < threshold) + price breaks above upper BB + volume surge
  - [WEIS] If phase = ACCUMULATION → squeeze resolving into Markup. Maximum conviction.
  - [DALTON] If breakout occurs from a multi-session bracket/balance area → "trade you almost
    have to do." Balance-area breakout + squeeze = explosive move.
  - 📝 Hint: "The spring is released from a value zone. Every lens agrees."

**Rule 3.3** [MURPHY + DALTON] MA Confirmation BUY with Value Location
  - [MURPHY] Price crosses above 20 SMA + 20 SMA > 50 SMA + RSI > 50
  - [DALTON] Entry is near or below developing VA → good trade location
  - [CONFLUENCE with WEIS] Stronger if wave balance = DEMAND_DOMINANT
  - 📝 Hint: "Trend healthy, entering at a fair price. Not chasing."

**Rule 3.4** [WEIS] Absorption BUY — Smart Money Accumulating
  - [WEIS] Rising supports + heavy volume at resistance absorbed + bag-holding at support
  - Needs 2 of 3 clues to confirm
  - [DALTON] This often manifests as overlapping Value Areas (bracket) with POC slowly rising
  - [CONFLUENCE] Absorption at bracket low + rising POC = institutional accumulation
  - 📝 Hint: "Smart money quietly loading inside a range."

**Rule 3.5** [DALTON — Market Profile Layer] Value-Area Rule BUY
  - Market opens outside previous VA → gets accepted BACK into VA (double TPOs / double prints)
  - **Action: BUY for full VA traverse to the opposite side.**
  - Probability increases when: closer to value, narrower VA, with-trend direction
  - [CONFLUENCE with MURPHY] If MAs point up and VA Rule triggers for upside traverse = HIGH prob
  - 📝 Hint: "Opened below value, got pulled back in. Now it'll likely cross the whole VA."

**Rule 3.6** [DALTON — Market Profile Layer] 3-to-I Day BUY
  - Initiative tail (buying) + Initiative TPO count (more above POC) + Initiative range extension
    (upward) all pointing UP
  - **94% next day opens within or better. 97% closes within or better.**
  - **MECHANICAL entry at or near VA Low. Stop below day's low.**
  - 📝 Hint: "Everything initiative points up. This is a 94%+ probability trade."

**Rule 3.7** [DALTON — Market Profile Layer] Balance-Area Breakout BUY
  - Multiple sessions of overlapping value (bracket) → breakout above bracket high
  - [DALTON] "A trade you almost have to do." Risk minimal, profit potential very high.
  - Often preceded by a failed auction below bracket low (outside day pattern)
  - [CONFLUENCE with BOLLINGER] If squeeze was active during the bracket → breakout is explosive
  - [CONFLUENCE with WEIS] If spring occurred at bracket low before the upside breakout = maximum
  - 📝 Hint: "The range is broken to the upside. Every trapped short fuels the move."

**Rule 3.8** [DALTON — Market Profile Layer] Gap BUY
  - Gap up open outside previous range = market out of balance
  - [DALTON] Trade WITH the initiative that caused the gap
  - Bigger gap = smaller odds of fill = STRONGER signal
  - If gap holds through first hour → continuation very likely
  - Stop at gap erasure point (where gap would fill)
  - [DALTON] Gap = break-away (strongest), acceleration (confirming), or exhaustion (weakest)
  - Determine gap type from context: first gap after bracket = break-away. Gap after extended
    move with weakening structure = possible exhaustion.
  - 📝 Hint: "A big gap that holds = someone big made a move overnight. Go with it."

**Rule 3.9** [CONFLUENCE — ALL FIVE] Maximum Conviction BUY
  - ALL of these simultaneously true:
    1. [DALTON] Good trade location (at or below developing VA/POC); OR balance-area breakout
    2. [WEIS] Phase = late ACCUMULATION or early MARKUP; wave balance = DEMAND_DOMINANT
    3. [MURPHY] TA Score > +45; momentum positive; volume confirming
    4. [BOLLINGER] Squeeze active or just broke out upward
    5. [BROOKS] Always-in LONG; recent bullish signal bar/pattern
  - **Action: Full position size. This is the rarest, highest-probability signal.**
  - 📝 Hint: "EVERY lens says BUY. This is as good as it gets."

### B. SELL / SHORT Entries

**Rule 3.10** [CONFLUENCE — WEIS + BROOKS + DALTON] Upthrust SELL (Maximum Confluence)
  - [WEIS] Price breaks above resistance → reverses → LOW volume on penetration
  - [BROOKS] Same event = failed breakout above range
  - [DALTON] Upthrust occurs at or near Value Area High → best institutional sell location
  - [DALTON] If at bracket high (multi-session resistance) → HIGHEST conviction
  - [WEIS] Follow-through: Next 1-3 bars DOWN. No follow-through = disqualified.
  - [WEIS] Trend context: Upthrust in DOWNTREND = highest success. In uptrend = risky.
  - 📝 Hint: "A fake breakout above resistance at an institutional value extreme."

**Rule 3.11** [DALTON — Market Profile Layer] Value-Area Rule SELL
  - Market opens above previous VA → accepted back into VA (double prints)
  - **SELL for full VA traverse to the downside.**

**Rule 3.12** [DALTON — Market Profile Layer] 3-to-I Day SELL
  - Initiative tail (selling) + Initiative TPO count (more below POC) + Initiative range extension
    (downward) all pointing DOWN
  - **MECHANICAL entry near VA High. Stop above day's high.**

**Rule 3.13** [DALTON — Market Profile Layer] Balance-Area Breakout SELL
  - Multiple sessions of overlapping value → breakout below bracket low
  - "A trade you almost have to do." Often preceded by a failed rally above bracket high.

**Rule 3.14** [DALTON — Market Profile Layer] Gap SELL
  - Gap down open outside previous range. Trade with the gap.
  - Stop at gap erasure point.

**Rule 3.15** [CONFLUENCE — ALL FIVE] Maximum Conviction SELL
  - ALL simultaneously: [DALTON] good sell location / [WEIS] DISTRIBUTION+SUPPLY /
    [MURPHY] bearish / [BOLLINGER] squeeze down or lower band break / [BROOKS] always-in SHORT
  - **Full position size.**

### C. DALTON-SPECIFIC ENTRY SIGNALS (new additions)

**Rule 3.16** [DALTON — Market Profile Layer] Neutral-Extreme Follow-Through
  - Previous day was a Neutral day with close on one extreme
  - **92% probability** next session opens within or better than VA
  - If close was on the HIGH extreme → BUY within VA on open
  - If close was on the LOW extreme → SELL within VA on open
  - 📝 Hint: "Neutral day with conviction at the close = 92% reliability the next morning."

**Rule 3.17** [DALTON — Market Profile Layer] Poor Low Retest BUY
  - Previous session(s) formed poor lows (no buying tail at the low)
  - Market revisits the poor low area with declining/lower volume
  - Poor low gets "repaired" — buying tail now forms
  - [CONFLUENCE with WEIS] If this is also a spring of the poor-low level → maximum confluence
  - 📝 Hint: "An unfinished low is being revisited. If buyers now show up, it's confirmed."

**Rule 3.18** [DALTON — Market Profile Layer] Open-Drive Momentum Entry
  - Market opens with an Open-Drive (immediate directional move)
  - Enter early in the direction of the drive
  - [DALTON] Do NOT wait for structural confirmation — "by the time you see it, it's too late"
  - If market returns through the opening price → conditions changed → EXIT
  - 📝 Hint: "Open-Drive = highest conviction open. Get on board NOW."


## 4. EXIT RULES
## ═════════════════════════════════════════════════════

### A. Exit LONG Positions

**Rule 4.1** [CONFLUENCE — WEIS + BOLLINGER + BROOKS + DALTON] Climax + Exhaustion Exit
  - [WEIS] Buying Climax: Extreme vol + wide up bar + close near LOW
  - [WEIS] Shortening of upward thrust: Last 3 up-waves cover less ground
  - [WEIS] Change in Behavior: Largest down-bar in 20 periods
  - [BOLLINGER] SAR flips from below to above price
  - [BROOKS] Two-leg move target reached
  - [DALTON] Profile becomes TOO elongated to the upside → inventory extreme → reversal imminent
  - [DALTON] Multiple sessions of poor highs (no selling tail) → structural weakness accumulating
  - Binary: ANY fires = tighten stops. TWO or more = EXIT NOW.
  - 📝 Hint: "When exhaustion signals pile up, don't be the last one out."

**Rule 4.2** [DALTON — Market Profile Layer] Short-Covering Rally Exit (for shorts)
  - P-shaped profile forming: activity concentrated in upper portion after decline
  - [DALTON] This is OLD business covering, NOT new buying
  - Successive higher auctions weakening (less range extension, less volume)
  - **Once rally stalls, the prior decline resumes → offers SELLING opportunity**
  - 📝 Hint: "The rally is just shorts covering. When they're done, it drops again."

**Rule 4.3** [DALTON — Market Profile Layer] Long-Liquidation Break Exit (for longs)
  - b-shaped profile forming: activity concentrated in lower portion after advance
  - This is OLD business liquidating longs, NOT new selling
  - **Once liquidation ends, the prior advance resumes → offers BUYING opportunity**
  - 📝 Hint: "The drop is just longs dumping. When they're done, it bounces."

**Rule 4.4** [WEIS] Distribution Phase Transition Exit
  - Phase shifts from MARKUP to DISTRIBUTION → tighten stops, scale out, no new longs
  - [DALTON] VAs begin to overlap → trend → bracket transition underway

**Rule 4.5** [DALTON — Market Profile Layer] Destination Reached Exit
  - If the initial trade was based on a destination trade (bracket low → bracket high,
    previous day's high, top of gap, etc.), take profits at or near the destination.
  - [DALTON] "Destinations don't have to be reached, but they serve as benchmarks."
  - 📝 Hint: "Price reached the destination you planned for. Take the money."

**Rule 4.6** [DALTON — Market Profile Layer] Timeframe Transition Exit
  - Was in a one-timeframe trade (trending). Double TPOs appear against the direction
    → transition from one-TF to two-TF is confirmed.
  - **Tighten stops significantly.** The directional edge has diminished.

**Rule 4.7** [DALTON — Market Profile Layer] Auction Failure Exit
  - Price auctions beyond a key reference and fails to follow through
  - [DALTON] The failure at the reference → swift move in the opposite direction
  - If you're positioned in the direction of the failed auction → EXIT IMMEDIATELY
  - Magnitude of reversal scales with significance of the reference tested
  - 📝 Hint: "It tried to break through and couldn't. Get out before the snapback."

### B. Exit SHORT Positions

**Rule 4.8** [WEIS] Selling Climax Exit (Cover Short)
  - Extreme volume + wide down bar + close near HIGH → smart money buying the panic
  - [DALTON] If accompanied by long-term excess low (tail, island, gap) → cover immediately

**Rule 4.9** [WEIS] Shortening of Downward Thrust + Accumulation Transition
  - Last 3 down-waves cover less distance → covering urgency increases
  - [DALTON] VAs stop moving lower; begin overlapping → bracket forming at bottom

**Rule 4.10** [MURPHY] Triple System Flip Exit
  - Triple Engine verdict changes from BUY to HOLD/SELL (or SELL to HOLD/BUY) → exit


## 5. WYCKOFF PHASE + DALTON MARKET STRUCTURE IDENTIFICATION
## ═════════════════════════════════════════════════════

### Rule 5.1 [CONFLUENCE — WEIS + DALTON] ACCUMULATION (Smart Money Buying)

**Weis view:**
- Trading range after significant decline
- Down-wave volume DECREASING, up-wave volume INCREASING
- Key events: Selling Climax → Spring → Test → Absorption → SOS

**Dalton view:**
- Multiple sessions of overlapping VAs (bracket)
- VAs at the BOTTOM of a longer-term range
- Poor lows getting tested and repaired
- Responsive buying below VA gaining momentum
- Initiative buying begins to appear

**CONFLUENCE:** When BOTH views align — Weis sees accumulation volume patterns AND Dalton sees bracketed VAs at lows with responsive-to-initiative buying transition → MAXIMUM CONVICTION that accumulation is real.

### Rule 5.2 [CONFLUENCE — WEIS + DALTON] MARKUP (The Uptrend)

**Weis view:**
- Higher highs, higher lows. Volume expands on rallies, contracts on pullbacks.

**Dalton view:**
- VAs separating and moving higher — trend = value diverging from price
- One-timeframing up on weekly timeframe (each week's low > prior week's low)
- Trend days appearing (elongated thin profiles)
- Responsive selling fails → initiative buying dominates

**CONFLUENCE exhaustion signals (LATE MARKUP):**
- [WEIS] Shortening of thrust + buying climax
- [DALTON] VAs begin overlapping again + poor highs appearing + profile becoming too elongated
- Both views agree = exit signal / do not add to longs

### Rule 5.3 [CONFLUENCE — WEIS + DALTON] DISTRIBUTION (Smart Money Selling)

**Weis view:**
- Trading range after significant advance
- Up-wave volume DECREASING, down-wave volume INCREASING
- Key events: Buying Climax → Upthrust → Absorption (bearish) → SOW

**Dalton view:**
- Multiple sessions of overlapping VAs (bracket) at the TOP of broader range
- Initiative selling appears; responsive buying weakens
- Poor highs accumulating

### Rule 5.4 [CONFLUENCE — WEIS + DALTON] MARKDOWN (The Downtrend)

**Weis view:**
- Lower highs, lower lows. Volume expands on declines, contracts on rallies.

**Dalton view:**
- VAs separating lower. One-timeframing down (each week's high < prior).
- Trend days to downside. Short-covering rallies (P-formations) = sell opportunity.


## 6. VOLUME READING RULES
## ═════════════════════════════════════════════════════

**Rule 6.1** [WEIS] The Master Principle: Effort vs Result
  - Volume = EFFORT. Price movement = RESULT.
  - HIGH effort + HIGH result = GENUINE move [MURPHY confirms] [DALTON: elongated profile]
  - HIGH effort + LOW result = ABSORPTION [DALTON: seen as wide profile at one price level]
  - LOW effort + HIGH result = NO OPPOSITION
  - LOW effort + LOW result = NO INTEREST [DALTON: Nontrend day]

**Rule 6.2** [WEIS] Demand/Supply Bar Reading
  - NO DEMAND: Up bar + LOW volume + NARROW spread
  - NO SUPPLY: Down bar + LOW volume + NARROW spread
  - [BROOKS] Narrow after a move = loss of momentum (confirming)

**Rule 6.3** [WEIS] Climax Volume
  - BUYING CLIMAX: Extreme vol + wide up + close near LOW → EXIT longs
  - SELLING CLIMAX: Extreme vol + wide down + close near HIGH → cover shorts
  - [DALTON] Climax days often produce excess (tails/gaps) that mark turning points

**Rule 6.4** [CONFLUENCE — WEIS + BOLLINGER + DALTON] Volume Dry-Up = Imminent Move
  - [WEIS] Volume < 50% of average, tiny range
  - [BOLLINGER] BBW near minimum → maximum compression
  - [DALTON] This forms a Nontrend day profile → bracket → breakout coming
  - Direction from Phase + Trend + Always-In
  - 📝 Hint: "Calm before the storm. All five lenses tell you which way it blows."

**Rule 6.5** [DALTON — Market Profile Layer] Volume at Price (Value Lens)
  - High-volume price levels = ATTRACTION ZONES (price slows here, rotational trade)
  - Low-volume price levels = REJECTION ZONES (price speeds through, acts as barrier)
  - [CONFLUENCE with WEIS] High volume AT a specific price (Dalton) + effort-vs-result reading
    of the move INTO that zone (Weis) = complete picture
  - 📝 Hint: "Where was the most business done? That's where price gravitates.
    Where was the LEAST? That's the wall to trade in front of."


## 7. CONFLICT RESOLUTION — When Lenses Disagree
## ═════════════════════════════════════════════════════

### Priority Hierarchy (from highest to lowest):

**Rule 7.1** [CONFLUENCE] Phase + Volume > Short-Term Indicators
  - [WEIS] phase + wave balance reflect weeks of behavior
  - [MURPHY] RSI/MACD reflect hours
  - If they conflict → trust the phase + volume

**Rule 7.2** [CONFLUENCE] Institutional Value > Indicator Signals
  - [DALTON] Value Area placement + trend/bracket status give institutional context
  - If [DALTON] says we're in a bracket (no trend) but [MURPHY] MAs say trending → trust Dalton
  - Markets are bracketed 70-80% of the time. MAs will whipsaw in brackets.
  - If [DALTON] says we're trending (VAs separating) → trust the trend, add on dips

**Rule 7.3** [CONFLUENCE] Bar Reading > Moving Averages
  - [BROOKS] strong reversal bar + [WEIS] effort/result at key level → leads MAs
  - [DALTON] Auction failure at reference → immediate action, don't wait for MA confirmation

**Rule 7.4** [CONFLUENCE] Spring/Upthrust WITH Follow-Through > All
  - [WEIS] A confirmed spring/upthrust with follow-through is the single strongest event
  - Overrides all other signals — even if MAs, Bollinger, etc. are neutral
  - BUT: no follow-through = disqualified regardless of everything else

**Rule 7.5** [CONFLUENCE] Bollinger Squeeze + Weis Phase + Dalton Bracket = Perfect Storm
  - [BOLLINGER] Squeeze (WHEN the move happens)
  - [WEIS] Phase (smart money direction)
  - [DALTON] Bracket about to break (institutional value shifting)
  - [BROOKS] Always-in confirms direction
  - All agree → Maximum timing conviction

**Rule 7.6** [DALTON — Market Profile Layer] One-Timeframing Overrides Counter-Signals
  - If the market is one-timeframing in a direction → do NOT trade against it
  - Even if [MURPHY] oscillators say overbought/oversold → one-timeframing wins
  - [DALTON] "It is generally financially dangerous to trade counter to one-timeframing."

**Rule 7.7** [CONFLUENCE] Change-in-Behavior + Poor Highs/Lows = Early Warning Override
  - [WEIS] CIB bar (largest opposite-direction bar) + [DALTON] poor highs/lows accumulating
  - Together = strong early warning of reversal
  - Action: Reduce size, tighten stops immediately, even if trend indicators say hold

**Rule 7.8** [CONFLUENCE] Conflicting Lenses = Reduce or Skip
  - 5/5 agree → full size, widest appropriate stops
  - 4/5 agree → 75% size, normal stops
  - 3/5 agree → 50% size, tight stops (marginal — consider skipping)
  - ≤2/5 → DO NOT TRADE

**Rule 7.9** [CONFLUENCE] Volume Is the Lie Detector
  - When in doubt about ANY signal → check volume
  - Signal + high volume = REAL
  - Signal + low volume = SUSPICIOUS
  - [DALTON] Also check: Signal + value developing in the signal's direction = REAL
  - [DALTON] Signal against value development (VA moving opposite) = SUSPICIOUS


## 8. DALTON-SPECIFIC RULES THAT ADD NEW CAPABILITY
## ═════════════════════════════════════════════════════
##
## These rules are NEW additions from Dalton that do not exist in
## any of the other four books. They add the institutional value layer.

**Rule 8.1** [DALTON — Market Profile Layer] Overnight Inventory Adjustment
  - Assess overnight inventory (long/short/neutral) BEFORE the session
  - If long overnight → expect early selling (adjustment). If market doesn't sell → very bullish.
  - If short overnight → expect early buying. If market doesn't buy → very bearish.
  - **The NON-ADJUSTMENT is the powerful signal.**
  - This concept appears in no other book in the system.

**Rule 8.2** [DALTON — Market Profile Layer] POC Migration as Leading Indicator
  - Track the developing POC during the session
  - POC migrating up = OTF buying control, value building higher
  - POC stagnant despite price rally = rally is short-covering, not genuine
  - POC migrating down = OTF selling control
  - [CONFLUENCE with WEIS] POC migration + wave volume = double confirmation of control
  - This concept adds real-time institutional intent tracking not in other books.

**Rule 8.3** [DALTON — Market Profile Layer] VA Placement Sequence
  - Track daily VA placement: higher/lower/overlapping across sessions
  - Higher VAs + higher volume = healthy uptrend [DALTON's 30-relationship table]
  - Higher VAs + lower volume = weak uptrend (caution)
  - Overlapping VAs = bracket forming (trade responsively)
  - VAs separating = trend resuming (trade with the momentum)
  - This adds a multi-session institutional value-flow perspective.

**Rule 8.4** [DALTON — Market Profile Layer] Bracket Test Count
  - Markets test bracket extremes 3-5 times on average before breaking
  - Track: How many times has the current bracket high/low been tested?
  - 1-2 tests: Likely to remain in bracket → trade responsively
  - 3-5 tests: Breakout probability increasing → prepare to go with
  - >5 tests: Extended bracket, breakout likely explosive when it comes

**Rule 8.5** [DALTON — Market Profile Layer] Poor High/Low Tracking
  - No buying tail (poor low) = auction incomplete → WILL be revisited
  - No selling tail (poor high) = auction incomplete → WILL be revisited
  - Single poor low = one data point (monitor)
  - Multiple consecutive poor lows = EXPONENTIAL RISK for longs
  - **Act: Reduce long exposure when multiple poor lows accumulate.**
  - Once revisited and a tail forms → auction complete → support validated

**Rule 8.6** [DALTON — Market Profile Layer] Destination Trade Framework
  - Once a directional auction starts, identify the destination:
    - Day TF: Previous day's high/low, opposite end of today's range, top/bottom of gap
    - Short-term: Bracket high/low, weekly high/low
    - Intermediate: Monthly high/low, multi-month bracket extreme
  - Enter trade → monitor for continuation → trail stop → take profits at destination
  - Destinations serve as profit targets AND location anchors for new entries

**Rule 8.7** [DALTON — Market Profile Layer] News/Auction Matrix
  - When major news hits, evaluate using 8-scenario matrix (Rule D55):
  - Market response AGAINST the news direction = VERY STRONG/VERY WEAK sentiment signal
  - Market response WITH the news = expected, no edge
  - **The non-obvious reaction is the informative one.**
  - [CONFLUENCE with WEIS] Non-obvious response + volume surge = double confirmation

**Rule 8.8** [DALTON — Market Profile Layer] Day Type Targeting
  - If Nontrend day developing → STAND ASIDE. No facilitation = no opportunity.
  - If Trend day developing → Get on board EARLY. Do NOT fade.
  - If Neutral day → Trade value area extremes responsively.
  - If Normal/NV → Two-sided trade within/near IB; opportunity on range extension side.
  - [BROOKS] Combine day type with always-in: Trend day + always-in = maximum momentum.


## 9. CONFLUENCE MAP — Where Books Confirm Each Other
## ═════════════════════════════════════════════════════

### A. SUPPORT/RESISTANCE CONFLUENCE

| Dalton Concept | Confirms | Confluence Insight |
|---|---|---|
| VA High/Low | [MURPHY] S/R levels | Institutional VA edges + classical S/R = strongest zones |
| POC (fairest price) | [MURPHY] Moving averages | POC near 20/50 SMA = double-confirmed fair value |
| Low-volume areas | [BOLLINGER] Band edges | Low-vol rejection zones + outer BB = mean-reversion anchor |
| Bracket extreme | [WEIS] Trading range edge | Bracket high/low = where springs/upthrusts happen |
| Single prints | [WEIS] Low-vol rejection | Single prints (Dalton) = low-vol zones (Weis) = barriers |

### B. TREND/PHASE CONFLUENCE

| Dalton Concept | Confirms | Confluence Insight |
|---|---|---|
| VAs separating higher | [MURPHY] Uptrend | Institutional value shifting up = MA-confirmed trend |
| One-timeframing up | [BROOKS] Always-in LONG | Successive auctions all UP = PA bias LONG |
| Bracket (overlap) | [WEIS] Accumulation/Distribution | Bracketed VAs = Wyckoff phase (loading/unloading) |
| Trend day profile | [MURPHY] Strong trend signal | Thin elongated profile = powerful trend conviction |
| Bracket→Trend transition | [WEIS] Markup/Markdown start | VA breakout = phase transition |

### C. ENTRY TIMING CONFLUENCE

| Dalton Concept | Confirms | Confluence Insight |
|---|---|---|
| 3-to-I day | [BROOKS] Strong momentum | All initiative signals aligned = enter mechanically |
| Balance-area breakout | [BOLLINGER] Squeeze breakout | Bracket break + band expansion = explosive |
| Value-Area Rule | [WEIS] Spring/Test | Accepted back into VA + spring pattern = double entry signal |
| Open-Drive | [BROOKS] Momentum entry | Strongest open type + always-in agreement = enter NOW |
| Gap (unfilled) | [WEIS] Excess / initiative | Gap = strongest excess + initiative activity = go with |

### D. EXIT/WARNING CONFLUENCE

| Dalton Concept | Confirms | Confluence Insight |
|---|---|---|
| Poor highs accumulating | [WEIS] Change in behavior | No selling tails + CIB bar = EXIT warning amplified |
| Profile too elongated | [WEIS] Buying climax | Extreme one-sided inventory + climax volume = reversal |
| P-formation (short cover) | [WEIS] Low-vol rally | Rally is covering, not new business = sell opportunity |
| Auction failure | [WEIS] Failed breakout | Failed reference test = spring/upthrust equivalent |
| VA overlap returning | [WEIS] Phase transition | VAs overlapping after trend = DISTRIBUTION starting |


## 10. SCORING INTEGRATION
## ═════════════════════════════════════════════════════
##
## The five-lens system now maps to the numerical Triple engine:
##
## BB Score:    -100 to +100  (4 Bollinger Methods)     [BOLLINGER]
## TA Score:    -100 to +100  (6 Murphy Categories)     [MURPHY]
## PA Score:    -100 to +100  (8 Brooks Components)     [BROOKS]
## Cross-Val:    -90 to  +90  (Agreement + Wyckoff + Dalton) [CONFLUENCE]
##   └─ Base agreement:  -60 to +60
##   └─ Wyckoff/Weis:    -30 to +30                     [WEIS]
##     └─ Phase bias:        ±8
##     └─ Events (SP/UT/SC/BC): ±6 each
##     └─ Absorption:        ±5
##     └─ Change-in-Behavior: ±4
##     └─ Wave balance:      ±3
##     └─ Shortening/SOT:    ±2
##     └─ Follow-through:    quality modifier on events
##
## DALTON INTEGRATION PATH:
##   Dalton's concepts enhance the cross-validation layer by providing
##   institutional VALUE context that strengthens or weakens signals:
##
##   Dalton factors within Wyckoff/Cross-Val scoring:
##     └─ VA placement sequence (higher/lower/overlap): modifies trend confidence
##     └─ Poor high/low count: adds warning weight against the weak direction
##     └─ Bracket test count: modifies breakout probability
##     └─ Day type compatibility: filters out low-probability setups
##     └─ Initiative vs responsive: modifies conviction on entries
##
##   NOTE: Dalton does NOT add a new independent score pillar.
##   Like Wyckoff/Weis, Dalton operates as an ENHANCER in cross-validation,
##   adding institutional context that the three core scores (BB, TA, PA)
##   cannot capture from OHLCV data alone.
##
## TOTAL:      -390 to +390 (unchanged)
##
## The 3 core scores (BB, TA, PA) are NEVER modified by Wyckoff or Dalton.
## Both operate ONLY in the cross-validation layer.


## APPENDIX A: QUICK REFERENCE — Five Questions Wallet Card
## ═════════════════════════════════════════════════════

```
┌──────────────────────────────────────────────────────────────┐
│  FIVE QUESTIONS BEFORE EVERY TRADE (< 60 seconds)           │
│                                                              │
│  Q1 [DALTON]    Where is institutional value?                │
│     → Am I at/near VA edge or POC? Initiative or responsive? │
│     → Trade location: GOOD / POOR                            │
│                                                              │
│  Q2 [WEIS]      What is the volume intent?                   │
│     → Phase: ACC / MU / DIST / MD                            │
│     → Wave balance: DEMAND / SUPPLY / BALANCED               │
│     → Exhaustion or CIB present? YES / NO                    │
│                                                              │
│  Q3 [MURPHY]    What is the macro structure?                 │
│     → MAs aligned? RSI/MACD confirming? S/R clear?           │
│     → Trend: UP / DOWN / FLAT                                │
│                                                              │
│  Q4 [BOLLINGER] What is the volatility state?                │
│     → Squeeze? Band position? Expansion/contraction?         │
│     → State: SQUEEZE / NORMAL / EXPANDED                     │
│                                                              │
│  Q5 [BROOKS]    What does price action say?                  │
│     → Always-in: LONG / SHORT / FLAT                         │
│     → Signal bar/pattern quality: HIGH / LOW                 │
│                                                              │
│  SCORING:                                                    │
│     5/5 = FULL SIZE    4/5 = 75%    3/5 = 50% (tight stops) │
│     ≤2/5 = DO NOT TRADE                                      │
└──────────────────────────────────────────────────────────────┘
```


## APPENDIX B: QUICK REFERENCE — Pre-Session Wallet Card
## ═════════════════════════════════════════════════════

```
┌──────────────────────────────────────────────────────────────┐
│  PRE-SESSION CHECKLIST (< 5 minutes)                        │
│                                                              │
│  □ Step 1: Monthly → Weekly → Daily bar review          60s  │
│    Long-term trend: UP / DOWN / BRACKET                      │
│    Key references marked: ___, ___, ___                      │
│                                                              │
│  □ Step 2: Three scenarios written down                 60s  │
│    If UP → action: ___                                       │
│    If DOWN → action: ___                                     │
│    If FLAT → action: ___                                     │
│                                                              │
│  □ Step 3: Overnight inventory assessed                 30s  │
│    Overnight: LONG / SHORT / NEUTRAL                         │
│    Expect adjustment? YES / NO                               │
│                                                              │
│  □ Step 4: Wyckoff phase + volume named                 60s  │
│    Phase: ACC / MU / DIST / MD                               │
│    Wave balance: DEMAND / SUPPLY / BALANCED                  │
│    SOT or CIB present? ___                                   │
│                                                              │
│  □ Step 5: Volatility state checked                     30s  │
│    State: SQUEEZE / NORMAL / EXPANDED                        │
│    BBW percentile: ___                                       │
│                                                              │
│  □ Step 6: Always-in direction determined               30s  │
│    Always-in: LONG / SHORT / FLAT                            │
│                                                              │
│  TOTAL: ~5 minutes. Now I'm prepared for any scenario.       │
└──────────────────────────────────────────────────────────────┘
```


## APPENDIX C: DALTON DAY-TYPE DECISION TREE
## ═════════════════════════════════════════════════════

```
Open Assessment:
│
├─ Open-DRIVE → Highest conviction
│   └─ Enter immediately in drive direction
│   └─ Expected: Trend Day or DD Trend Day
│   └─ If price returns through open → EXIT (conditions changed)
│
├─ Open-TEST-DRIVE → High conviction
│   └─ Enter after test fails and reversal confirmed
│   └─ Expected: Normal Variation or Trend Day
│
├─ Open-REJECTION-REVERSE → Moderate conviction
│   └─ Be PATIENT. Wait for rotation back toward value.
│   └─ Expected: Normal or Normal Variation
│
├─ Open-AUCTION (In Range) → Low conviction
│   └─ Trade VA extremes responsively OR stand aside
│   └─ Expected: Nontrend, Normal, or Neutral
│
└─ Open-AUCTION (Out of Range) → Unknown conviction
    └─ Market out of balance but open isn't directional
    └─ Be alert — potential for big directional day
    └─ Expected: Could become any day type
```


## APPENDIX D: THE 30 DIRECTIONAL PERFORMANCE RELATIONSHIPS
## ═════════════════════════════════════════════════════
##
## From Dalton Chapter 4, Section II. Evaluate daily.
##
## Format: Attempted Direction + Volume + VA Placement = Performance

```
ATTEMPTING UP:
  Higher Vol + Higher VA = VERY STRONG (healthy uptrend)
  Higher Vol + Overlapping VA = STRONG (advancing but contested)
  Higher Vol + Lower VA = NEUTRAL (vol up but value down — mixed)
  Higher Vol + Unchanged VA = MODERATE (volume but no value shift)
  Lower Vol + Higher VA = MODERATE (value up but weak vol — caution)
  Lower Vol + Overlapping VA = WEAK (poor facilitation)
  Lower Vol + Lower VA = VERY WEAK (failing on all counts)
  Lower Vol + Unchanged VA = WEAK
  Unchanged Vol + Higher VA = MODERATE
  Unchanged Vol + Overlapping VA = NEUTRAL
  Unchanged Vol + Lower VA = WEAK
  Unchanged Vol + Unchanged VA = NEUTRAL (no change at all)

ATTEMPTING DOWN:
  Higher Vol + Lower VA = VERY STRONG (healthy downtrend)
  Higher Vol + Overlapping VA = STRONG (declining but contested)
  Higher Vol + Higher VA = NEUTRAL (vol up but value up — mixed)
  Higher Vol + Unchanged VA = MODERATE
  Lower Vol + Lower VA = MODERATE (value down but weak vol)
  Lower Vol + Overlapping VA = WEAK
  Lower Vol + Higher VA = VERY WEAK (failing on all counts)
  Lower Vol + Unchanged VA = WEAK
  Unchanged Vol + Lower VA = MODERATE
  Unchanged Vol + Overlapping VA = NEUTRAL
  Unchanged Vol + Higher VA = WEAK
  Unchanged Vol + Unchanged VA = NEUTRAL
```


## APPENDIX E: MARKETS TO AVOID CHECKLIST
## ═════════════════════════════════════════════════════

Before entering ANY trade, check:

```
□ Is this a Nontrend day? (narrow range, no range extension, low vol)     → SKIP
□ Is this a Nonconviction day? (normal-looking but no OTF references)     → SKIP
□ Is this a long-term nontrend market? (erratic, no directional VAs)      → DAY TRADE ONLY
□ Is major news imminent? (within 30 min of announcement)                  → FLATTEN/REDUCE
□ Is the Five-Question alignment ≤ 2/5?                                    → DO NOT TRADE
□ Did the pre-session checklist produce no clear scenarios?                → STAND ASIDE
```


---

*This unified system integrates rules from five books:*
*1. John Bollinger — "Bollinger on Bollinger Bands" [BOLLINGER]*
*2. John Murphy — "Technical Analysis of the Financial Markets" [MURPHY]*
*3. Al Brooks — "Trading Price Action" series [BROOKS]*
*4. David Weis — "Trades About to Happen" [WEIS]*
*5. James Dalton — "Mind Over Markets" (Updated Edition) [DALTON]*
*Extracted from thorough, cover-to-cover readings of all five books.*
